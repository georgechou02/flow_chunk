...
class MultiTaskDiTConfig(PreTrainedConfig):
    ...
    def _validate(self):
        ...
        if self.vision_encoder_type not in ["clip", "resnet"]:
            raise ValueError(
                f"vision_encoder_type must be 'clip' or 'resnet', got '{self.vision_encoder_type}'"
            )
        if self.vision_encoder_type == "clip" and "clip" not in self.vision_encoder_name.lower():
            raise ValueError(
                f"vision_encoder_name must be a CLIP model (contain 'clip'), got '{self.vision_encoder_name}'"
            )
        if self.vision_encoder_type == "resnet":
            if not self.vision_backbone.startswith("resnet"):
                raise ValueError(
                    f"vision_backbone must be a torchvision ResNet variant, got '{self.vision_backbone}'"
                )
            if self.spatial_softmax_num_keypoints <= 0:
                raise ValueError("spatial_softmax_num_keypoints must be positive")
        if (
