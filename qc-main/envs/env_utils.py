...
def make_env_and_datasets(
    ...
    Args:
        ...
        ratio: D4RL small-sample ratio. 1 uses the full dataset.
        small_samples_dir: Directory of subsampled D4RL npys (ratio != 1).

    Returns:
        A tuple of the environment, evaluation environment, training dataset, and validation dataset.
    """
    d4rl_dataset_kwargs = dict(ratio=ratio)
    if small_samples_dir is not None:
        d4rl_dataset_kwargs["small_samples_dir"] = small_samples_dir
    ...
    if 'singletask' in env_name:
        ...
        dataset = d4rl_utils.get_dataset(env, env_name, **d4rl_dataset_kwargs)
